//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@interface GDTAdService : NSObject
{
}

+ (id)baseUrlWithServerType:(long long)arg1;
+ (void)checkSettingUpdateWithSeq:(long long)arg1;
+ (long long)convertAdTypefromString:(id)arg1;
+ (void)doSendTrackEffectLogicOverall:(id)arg1 clickid:(id)arg2 sendTrackEffectType:(long long)arg3;
+ (void)doSendTrackEffectLogicWhenFailed:(id)arg1 clickid:(id)arg2 sendTrackEffectType:(long long)arg3;
+ (void)doSendTrackEffectLogicWhenSucceed:(id)arg1 clickid:(id)arg2 sendTrackEffectType:(long long)arg3;
+ (void)exposureAdWithAdModel:(id)arg1 replacementData:(id)arg2 success:(CDUnknownBlockType)arg3 failure:(CDUnknownBlockType)arg4;
+ (void)exposureAdWithAdModel:(id)arg1 success:(CDUnknownBlockType)arg2 failure:(CDUnknownBlockType)arg3;
+ (void)exposureAdWithURL:(id)arg1 threadId:(id)arg2 success:(CDUnknownBlockType)arg3 failure:(CDUnknownBlockType)arg4;
+ (void)exposureVideoAdWithURL:(id)arg1 viewId:(id)arg2 videoInfoDict:(id)arg3 threadId:(id)arg4 success:(CDUnknownBlockType)arg5 failure:(CDUnknownBlockType)arg6;
+ (void)fetchAdWithRequestData:(id)arg1 threadId:(id)arg2 retryTimes:(long long)arg3 success:(CDUnknownBlockType)arg4 notice:(CDUnknownBlockType)arg5 failure:(CDUnknownBlockType)arg6;
+ (_Bool)isPlacementIdOffline:(id)arg1;
+ (id)logoUrlWithAdModel:(id)arg1;
+ (void)newFetchAdWithRequestData:(id)arg1 threadId:(id)arg2 retryTimes:(long long)arg3 success:(CDUnknownBlockType)arg4 notice:(CDUnknownBlockType)arg5 failure:(CDUnknownBlockType)arg6;
+ (void)reportThirdPartyClickUrlWithList:(id)arg1 replacementData:(id)arg2 reportType:(long long)arg3;
+ (void)reportThirdPartyUrlWithList:(id)arg1 replacementData:(id)arg2 reportType:(long long)arg3;
+ (void)reportVastEventWithAdModel:(id)arg1 eventType:(long long)arg2 success:(CDUnknownBlockType)arg3 failure:(CDUnknownBlockType)arg4;
+ (void)sendClickTrackWithClickParam:(id)arg1 adModel:(id)arg2;
+ (void)sendEmptyOrderWithAdModel:(id)arg1;
+ (void)sendTrackEffectWithResult:(_Bool)arg1 clickParam:(id)arg2 sendTrackEffectType:(long long)arg3 deepLinkFailedCallback:(CDUnknownBlockType)arg4;
+ (id)urlWithClickManager:(id)arg1 clickParam:(id)arg2 adModel:(id)arg3;
+ (id)urlWithClickManager:(id)arg1 clickParam:(id)arg2 adModel:(id)arg3 replacementData:(id)arg4;

@end

